...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        if not env.__contains__(self.var_name):
            return None
        else:
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        "*** YOUR CODE HERE ***"
        #operator: Name('add')
        #operands: Literal(3) Name('a')
        #数据类型：Literal Name LambdaExpr //Number(specific)
        # and maybe nest CallExpr
        for operand in self.operands:
            operand = operand.eval(env)
            #if isinstance(operand, CallExpr):
                #operand = operand.eval(env) #return Number(n)
            #elif isinstance(operand,Literal):
                #operand = operand.eval(env)
            #elif isinstance(operand,Name):
                #operand = operand.eval(env)

        ls = [ operand.eval(env) for operand in self.operands]

        def compute(ls,self):
            #if len(ls) == 1:
                #return self.operator.eval(env).apply(ls[0])
            #elif len(ls) == 2:
                #temp = self.operator.eval(env)
                #return self.operator.eval(env).apply(ls)
            #else:
                #ls[0] = self.operator.eval(env).apply(ls[0],ls[1])
                #ls.pop(1)
                #return compute(ls,self)
            return self.operator.eval(env).apply(ls)
        return compute(ls,self)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        #现在就是处理body，body是一个Expr类，里面的参数用value这个字典来换成值
        #new_env = global_env.copy()
        #lambda 和普通的函数有什么区别，lambda 函数没有operator，
