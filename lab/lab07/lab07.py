def insert_into_all(item, nested_list):
    ...
    for lt in nested_list:
        ...
        new_newsted_list.append(new_lst)
    return new_newsted_list
...
def subseqs(s):
    ...
    [[]]
    """
    #use recursion ,insert into the first element
    if len(s)==0:
        return [[]]
    else:
        subset=subseqs(s[1:])
...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            a = inc_subseqs(s[1:0])
            b =[ls for ls in a if ls[0]>s[0]]
            return insert_into_all(s[0], b) +inc_subseqs(s[1:])
...
def make_advanced_counter_maker():
    ...
    def count():
        local_count=0
        def advance_count(stl):
            nonlocal global_counter,local_count
            if stl=='count':
                local_count+=1
                return local_count
            elif stl=='global-count':
                global_counter+=1
                return global_counter
            elif stl=='reset':
                local_count=0
            elif stl=='global-reset':
                global_counter=0
