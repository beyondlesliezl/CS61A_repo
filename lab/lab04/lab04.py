...
def add_chars(w1, w2):
    ...
    #始终比较w1 w2的第一个字母，所以就有三种情况
    #1. 首先是两者不相等，由于w2的长度要长于w1 那么直接返回w2的第一个字母再加剩下的
    #2. 两者相等，那么二者同时向后移一位，再进行比较
    #3.因为w1是短于w2的，所以有可能w1已经结束了，此时没有字母，len(w1)==0,那么就直接返回w2
