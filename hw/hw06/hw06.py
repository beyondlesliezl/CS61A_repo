...
class Mint:
    ...
    # The update method sets the year stamp to the
    ...
    # The create method takes a subclass of Coin and returns an
    # instance of that class stamped with the mint's year
    # (which may be different from Mint.current_year if it has not been updated.)
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        year_change = Mint.current_year - self.year
        return self.cents + (year_change - 50 if year_change - 50 > 0 else 0)

    # A Coin's worth method returns the cents value of the coin
    #  plus one extra cent for each year of age beyond 50.
    # A coin's age can be determined by subtracting the coin's year
    # from the current_year class attribute of the Mint class.
...
def is_bst(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    # 如何判断是否只有一个children branches ？ len(branches)==1
    # 大于或者小于子节点都是可以的
    #所有在右侧的节点都必须大于上一个节点，所有在左侧的节点都必须小于上一个节点。
    #而且所有在右侧的节点，如果只有一个节点的话，也必须大于最开始的节点
    #一个节点是很特殊的情况，他可以大于或者小于上一个节点，但是他如果是属于右侧的节点，就必须大于
    #对于左边右侧的节点，必须大于上一个节点，但是又必须小于上——上个节点，即有两个限制条件
    #对于右边，左侧的节点，必须小于上一个节点，但是又必须要大于上-上一个节点，也有两个限制条件
    # 1. at most 2 rbanches
    # 2. all[is_bst(b) for b in t. branches]==True
    # 3. bst_max(left)<=t.label
    # 4. bst_min(right)>t.label
    # what I learn : write down the demand 
    def bst_min(t):
        if t.is_leaf():
            return t.label
...
def remove_all(link, value):
    ...
    while link.rest.first == value:
        ...
        if link.rest == Link.empty:# important
            return 
