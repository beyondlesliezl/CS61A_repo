...
def balanced(m):
    ...
    if is_mobile(end(left(m))):
        lef=balanced(end(left(m)))
    if is_mobile(end(right(m))):
        rig=balanced(end(right(m)))
    return lef and rig and (length(left(m))*total_weight(end(left(m)))==length(right(m))*total_weight(end(right(m))))
...
def totals_tree(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    #return a list ,so that can use the print_tree
    if is_planet(m):
...
def replace_leaf(t, find_value, replace_value):
    ...
    """
    "*** YOUR CODE HERE ***"
    #it's means that the original is not modified,just only return a new tree
    #so just copy the original tree and replace the find_value 
    #但是这个是形参传递，所以不会改变它的值
    if is_leaf(t):
